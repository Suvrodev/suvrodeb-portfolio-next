...
import { useUpdateProjectMutation } from "@/redux/features/ProjectManagement/projectManagement";
...
const UpdateProject = ({ project }: IProps) => {
  ...
  // 🖼️ Handle image upload (base64 + compression)
  ...
  const onSubmit: SubmitHandler<TProjectForm> = async (data) => {
    ...
    if (res?.status) {
      okToast("Project Updated Successfully 🎉");
      setIsSuccess(true);
      handleRevalidate();
      setTimeout(() => setIsSuccess(false), 4000);
    } else {
      erToast("Failed to update project");
    }
  };
      ...
      >
        ...
        <div className="text-center mb-8 sm:mb-10">
          ...
          <p className="text-slate-400 text-sm sm:text-base">
            Modify your project details easily 🚀
          </p>
        </div>
          ...
          >
            {isLoading ? (
              ...
              <span className="flex items-center justify-center gap-2">
                <Sparkles className="w-5 h-5" /> Update Project
              </span>
            )}
          </motion.button>
        </form>
        ...
        <p className="text-center text-slate-500 text-xs sm:text-sm mt-8">
          Your project details have been updated successfully ✨
        </p>
      </motion.div>
    </div>
  );
