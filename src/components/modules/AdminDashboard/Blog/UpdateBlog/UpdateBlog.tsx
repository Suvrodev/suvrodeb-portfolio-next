...
import { useUpdateBlogMutation } from "@/redux/features/BlogManagement/blogmanagement";
...
const UpdateBlog = ({ blog }: IProps) => {
  ...
  const [preview, setPreview] = useState<string>(blog?.image || "");
  ...
  const onSubmit: SubmitHandler<TBlogForm> = async (data) => {
    ...
    loadingToast("Updating Blog...");

    console.log("Data: ", data);

    const res = await updateBlog({ id: blog?._id, updateData: data }).unwrap();
    console.log("Update Response:", res);

    if (res?.status) {
      okToast("Blog Updated Successfully 🎉");
      setIsSuccess(true);
      handleRevalidate();
      ...
      >
        ...
        <div className="text-center mb-8">
          ...
          <p className="text-slate-400 text-sm">
            Modify your blog and republish it 🚀
          </p>
        </div>
          ...
          >
            {isLoading ? (
              ...
              <span className="flex items-center justify-center gap-2">
                <Sparkles className="w-5 h-5" /> Update Blog
              </span>
            )}
          </motion.button>
        </form>
